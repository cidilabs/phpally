...
class ImageHasAltTest extends PhpAllyTestCase {
    ...
    {
        $html = '<div><img src="validDecorativeImage.png" data-decorative="true" role="presentation"></div>';
        $dom = new \DOMDocument('1.0', 'utf-8');
        $dom->loadHTML($html);
        $rule = new ImageHasAlt($dom);
    ...
    {
        $html = '<div><img src="validDecorativeImage.png" data-decorative="false" role="none"></div>';
        $dom = new \DOMDocument('1.0', 'utf-8');
        $dom->loadHTML($html);
        $rule = new ImageHasAlt($dom);
